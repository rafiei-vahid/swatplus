...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      integer :: isched !              |
      !! deliberately declared WITHOUT an initializer: an initializer would give iter_guard the
      !! SAVE attribute (F2018 8.5.16), i.e. one static copy shared by every thread, and this
      !! routine runs in the parallel land phase. It is set explicitly before the loop below.
      integer :: iter_guard !          |full-pass guard against an infinite same-date loop
