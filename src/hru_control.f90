...
!!    hydrologic cycle
      ...
      if (nd_30 > 30) nd_30 = 1
        ...
        !! swatplus_perf OpenMP: plant per-day scratch arrays are now threadprivate
        !! (uno3d/uapd/un2/up2/translt/par/htfac/epmax), so the plant phase runs in
        !! parallel across HRUs without the previous serializing critical.
        call pl_community
        !if (j == 136) then
        ...
        if (ires > 0) then
          ...
          if (wet_dat_c(ires)%hyd.eq.'paddy') then !.and.time%yrs > pco%nyskip) then
            ...
            !! swatplus_perf OpenMP: shared paddy-debug unit; serialize concurrent writes
            !$omp critical (paddy_dbg_out)
           write(100100,'(4(I6,","),20(f20.1,","))') time%yrc,time%mo,time%day_mo,j,w%precip,irrig(j)%applied,hru(j)%water_seep,     &
            ...
            !$omp end critical (paddy_dbg_out)
            end if
          end if
        end if
     ...
     !! compute reduction in pollutants due to in field grass waterway
      ...
      ! output_losses
        ...
        if (time%yrs > pco%nyskip) then
          !! swatplus_perf OpenMP: basin sediment-budget reduction across HRUs.
          !$omp atomic
