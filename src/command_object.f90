      subroutine command_object(ic)
!!    swatplus_perf OpenMP: reentrant per-object body extracted verbatim from
!!    command.f90 so it can be dispatched both serially and inside an !$omp
!!    parallel wave over independent HRUs. icmd is set = ic (threadprivate) at
!!    entry; all dispatch/inflow scratch globals are threadprivate; subroutine
!!    locals are stack-private per call. Behavior is identical to the inline body.
      use time_module
