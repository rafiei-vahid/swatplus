...
!     read-before-written and relied on cross-call persistence, so zeroing them each call shifted
!     PET -> soil water -> HRU loads -> in-channel chla ~0.3% vs the serial engine. Keeping them
!     SAVE + threadprivate gives each thread its own persistent copy: byte-identical at 1 thread,
!     race-free at N>1.
!$omp threadprivate(j, idp, iob, tk, pb, gma, xl, ea, ed, dlt, ramm, ralb1, ralb, xx, rbo, rto,   &
!$omp   rn, uzz, zz, zom, zov, rv, rn_pet, fvpd, rc, rho, rout, d, chz, gsi_adj, pet_alpha,         &
!$omp   gsi_wav, igrocom)
